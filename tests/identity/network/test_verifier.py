...
def test_agent_specific_range_can_bind_exact_agent() -> None:
    ...
    evidence = OfficialRangeVerifier().verify(
        ...
        source_profile=profile,
        document=document(
            "perplexity-user.json", "perplexity", profile.uri, profile.binding_scope
        ),
    )
...
def test_source_for_different_agent_is_not_applied() -> None:
    profile = provider_profile("openai").range_sources[0]
    evidence = OfficialRangeVerifier().verify(
        context=context("192.0.2.10", SourceAddressProvenance.DIRECT_PEER),
        claim=claim("openai", "OAI-SearchBot"),
        source_profile=profile,
        document=document("openai-gptbot.json", "openai", profile.uri, profile.binding_scope),
    )
