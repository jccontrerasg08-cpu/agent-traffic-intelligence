...
def response(
    ...
    **headers: str,
    ...
    return TransportResponse(status=status, headers=merged, body=body)
...
def test_redirect_without_location_is_rejected() -> None:
    resolver = FakeResolver({"example.com": ("93.184.216.34",)})
    transport = FakeTransport([response(302)])
    with pytest.raises(FetchProtocolError, match="Location"):
        SafeFetcher(resolver=resolver, transport=transport).fetch(
            "https://example.com/start"
        )
