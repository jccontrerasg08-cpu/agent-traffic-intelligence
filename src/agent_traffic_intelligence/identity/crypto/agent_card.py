...
class AgentCard:
    """Parsed metadata; fields remain self-asserted until authority binding succeeds."""
...
def parse_agent_card(
    ...
    *,
    ...
    """Parse the registry-03 CIMD-style card and ignore unknown extension members."""
    ...
    has_known_parameter = (
        bool(recognized_top_level.intersection(decoded))
        or bool(recognized_web_bot_auth.intersection(extension))
    )
    if not has_known_parameter:
    ...
    return AgentCard(
        ...
        robots_compliance=_string_tuple(
            extension.get("rfc9309-compliance"), "web_bot_auth.rfc9309-compliance"
        ),
        trigger=trigger,
