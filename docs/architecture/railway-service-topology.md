...
La PR #32 que contiene esta corrección superó la CI protegida y fue fusionada en `main`. Railway detectó el commit y comenzó automáticamente un segundo despliegue; durante la última comprobación visible estaba en fase `BUILDING`. La validación pública sólo se marcará como confirmada después de que ese despliegue quede `Online` y se pruebe un dominio generado para este servicio.
