...
| Un endpoint público necesita evitar ampliar la retención de datos por defecto. | RFC 7239 dedica consideraciones de privacidad a los campos de proxy; el contrato existente de ATI ya minimiza IPs, query strings y cuerpos. [2] | Confirmado para el riesgo; implementación por verificar | El servicio devolverá un resultado por solicitud, con datos agregados/derivados, sin escribir telemetría del visitante por defecto. |
